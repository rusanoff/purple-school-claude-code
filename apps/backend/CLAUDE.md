...
- **`pnpm --filter backend test`** — unit tests. Config is inline in `package.json` (`rootDir: "src"`, matches `*.spec.ts` sitting next to the code it tests). The suite is small and deliberately narrow: 13 tests across `src/meeting-files/constants/file-upload.constants.spec.ts`, `src/meeting-files/storage/meeting-file-storage.service.spec.ts` and `src/users/interfaces/user-profile.interface.spec.ts` — i.e. only pure functions and one service, no HTTP and no database. No handler has a unit test — `passWithNoTests: true` stays set in the jest config so the command keeps exiting `0` if the pattern ever matches nothing. Handlers are plain classes with constructor DI (see the CQRS section's **Handler contract**), so they're straightforward to unit test with a mocked `PrismaService`/`CommandBus`/`QueryBus` if that becomes worthwhile — add a `*.spec.ts` beside the handler.
